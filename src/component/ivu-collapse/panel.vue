<template>
  <div :class="[
    'ivu-collapse-item',
    {
      'ivu-collapse-item-active': this.isActive
    }
  ]">
    <div class="ivu-collapse-header" @click="toggle">
      <svg v-if="!hideArrow" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
        <path fill="currentColor" d="M12.8414611,12 L10.1203717,8.82539569 C9.94066062,8.61573277 9.96494139,8.30008277 10.1746043,8.1203717 C10.3842672,7.94066062 10.6999172,7.96494139 10.8796283,8.17460431 L13.8796283,11.6746043 C14.0401239,11.8618492 14.0401239,12.1381508 13.8796283,12.3253957 L10.8796283,15.8253957 C10.6999172,16.0350586 10.3842672,16.0593394 10.1746043,15.8796283 C9.96494139,15.6999172 9.94066062,15.3842672 10.1203717,15.1746043 L12.8414611,12 Z"/>
      </svg>
      <slot></slot>
    </div>
    <div class="ivu-collapse-content" v-show="isActive">
      <div class="ivu-collapse-box">
        <slot name="content"></slot>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  name: 'Panel',
  props: {
    name: {
      type: String,
    },
    hideArrow: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      index: 0,
      isActive: false,
      mounted: false
    }
  },
  methods: {
    toggle() {
      this.$parent.toggle({
        name: this.name || this.index,
        isActive: this.isActive
      })
    }
  },
  mounted() {
    this.mounted = true
  }
}
</script>